...
mkdir -p res
cd res
for j in BullyGB cgb-acid-hell cgb-acid2 dmg-acid2 Gambatte-testroms mealybug-tearoom-tests rtc3test SameSuite; do
...
dotnet test -a "$home/test_output" -c "$config" -l "junit;LogFilePath=$home/test_output/{assembly}.coverage.xml;MethodFormat=Class;FailureBodyFormat=Verbose" "$@"
